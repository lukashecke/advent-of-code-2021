...
for y in range(100):
    check = 0
    for x in range(100):
        if 0 < x < 99 and 0 < y < 99:  # all 'middle' numbers
        ...
        elif 0 < x < 99 and y == 0:  # all top 'middle' numbers
            if map[x][y] < map[x - 1][y]:  # left
                if map[x][y] < map[x + 1][y]:  # right
                    if map[x][y] < map[x][y + 1]:  # bottom
                        risk_level += map[x][y] + 1
        elif x == 0 and 0 < y < 99:  # all left 'middle' numbers
            if map[x][y] < map[x][y - 1]:  # top
                if map[x][y] < map[x + 1][y]:  # right
                    if map[x][y] < map[x][y + 1]:  # bottom
                        risk_level += map[x][y] + 1
        elif x == 99 and 0 < y < 99:  # all right 'middle' numbers
            if map[x][y] < map[x][y - 1]:  # top
                if map[x][y] < map[x - 1][y]:  # left
                    if map[x][y] < map[x][y + 1]:  # bottom
                        risk_level += map[x][y] + 1
        elif 0 < x < 99 and y == 99:  # all bottom 'middle' numbers
            if map[x][y] < map[x - 1][y]:  # left
                if map[x][y] < map[x][y - 1]:  # top
                    if map[x][y] < map[x + 1][y]:  # right
                        risk_level += map[x][y] + 1
